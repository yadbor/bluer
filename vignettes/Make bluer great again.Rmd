...
So, a \code{sample} consists of one or more \code{specimen}s. Each \code{specimen} is a single test. Each \code{sample} is the application of a single \code{test} protocol. All the \code{specimen}s in a \code{sample} have (notionally) the same \code{test} protocol.
